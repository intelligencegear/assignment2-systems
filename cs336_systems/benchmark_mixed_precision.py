...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="Transformer 混合精度性能基准测试")
    # 模型超参数（保持不变）
    ...
    parser.add_argument("--use_bf16", action="store_true", help="启用BF16混合精度")  # 新增
    ...
    # 定义精度上下文：启用BF16时用autocast，否则用空上下文
    ...
    # 输出结果（区分精度类型）
    precision_type = "BF16混合精度" if args.use_bf16 else "FP32全精度"
    step_type = "前向+反向传播" if args.include_backward else "前向传播"
    print(f"\n{precision_type} {step_type}性能统计:")
